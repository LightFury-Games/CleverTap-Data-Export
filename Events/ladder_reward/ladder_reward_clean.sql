CREATE OR REPLACE VIEW clevertap.ladder_screen_entered_events AS
...
SELECT
    ...
    -- ladder_screen_entered specific event properties
    element_at(eventProps, 'player_id').member5
        AS player_id,

    element_at(eventProps, 'player_name').member5
        AS player_name,

    element_at(eventProps, 'player_tag').member5
        AS player_tag,

    element_at(eventProps, 'ladder_is_ranked').member5
        AS ladder_is_ranked,

    element_at(eventProps, 'rank').member5
        AS rank,

    element_at(eventProps, 'stars').member5
        AS stars,

    element_at(eventProps, 'overall_rating').member1
        AS overall_rating,

    element_at(eventProps, 'games_played').member5
        AS games_played,
...
FROM clevertap.ladder_screen_entered;
