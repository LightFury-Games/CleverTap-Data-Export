CREATE OR REPLACE VIEW clevertap.device_profile_info_events AS
SELECT
    -- =========================================================
    -- EVENT
    -- =========================================================
    ...
    element_at(eventProps, 'device_tier').member5 AS device_tier,
    element_at(eventProps, 'device_model').member5 AS event_device_model,
    element_at(eventProps, 'device_profilename').member5 AS device_profile_name,
    element_at(eventProps, 'device_gpufamily').member5 AS device_gpu_family,

    element_at(eventProps, 'device_use_vulcan').member1 AS device_use_vulcan,
    ...
    element_at(deviceInfo, 'appVersion').member0 AS device_app_version,
    element_at(deviceInfo, 'platform').member0 AS device_platform,
    element_at(deviceInfo, 'osVersion').member0 AS os_version,
    element_at(deviceInfo, 'browser').member0 AS browser,
    element_at(deviceInfo, 'model').member0 AS device_model,
    element_at(deviceInfo, 'make').member0 AS device_make,

    TRY_CAST(
        element_at(deviceInfo, 'sdkVersion').member0 AS INTEGER
    ) AS sdk_version,

    TRY_CAST(
        element_at(deviceInfo, 'dpi').member0 AS INTEGER
    ) AS dpi,

    TRY_CAST(
        element_at(deviceInfo, 'width').member0 AS INTEGER
    ) AS device_width,

    TRY_CAST(
        element_at(deviceInfo, 'height').member0 AS INTEGER
    ) AS device_height,

    element_at(deviceInfo, 'unit').member0 AS device_dimension_unit,
...
FROM clevertap.device_profile_info;