...
CREATE OR REPLACE VIEW clevertap.store_viewed_events AS
...
SELECT
    ...
    TRY_CAST(
        element_at(
            eventProps,
            'total_sold_out_cards'
        ).member5
        AS INTEGER
    ) AS total_sold_out_cards,

    TRY_CAST(
        element_at(
            eventProps,
            'total_sold_out_card_packs'
        ).member5
        AS INTEGER
    ) AS total_sold_out_card_packs,
    ...
    TRY_CAST(
        element_at(
            eventProps,
            'total_card_pack_slots'
        ).member5
        AS INTEGER
    ) AS total_card_pack_slots,
...
FROM clevertap.store_viewed

WHERE eventName = 'store_viewed';
