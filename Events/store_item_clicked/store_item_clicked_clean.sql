CREATE OR REPLACE VIEW clevertap.store_item_clicked_events AS
...
SELECT
    ...
    -- store_item_clicked specific event properties
    element_at(eventProps, 'shop_id').member5
        AS shop_id,

    element_at(eventProps, 'bundle_id').member5
        AS bundle_id,

    element_at(eventProps, 'slot_index').member5
        AS slot_index,

    element_at(eventProps, 'currency_id').member5
        AS currency_id,

    element_at(eventProps, 'item_price').member5
        AS item_price,
...
FROM clevertap.store_item_clicked;
