CREATE OR REPLACE VIEW clevertap.task_completed_notified_events AS
...
SELECT
    ...
    -- task_completed_notified specific event properties
    element_at(eventProps, 'task_id').member5
        AS task_id,

    element_at(eventProps, 'task_board_id').member5
        AS task_board_id,

    element_at(eventProps, 'task_title').member5
        AS task_title,

    element_at(eventProps, 'task_description').member5
        AS task_description,

    element_at(eventProps, 'pool_id').member5
        AS pool_id,

    element_at(eventProps, 'bucket').member5
        AS bucket,

    element_at(eventProps, 'complexity').member5
        AS complexity,

    element_at(eventProps, 'difficulty').member5
        AS difficulty,
...
FROM clevertap.task_completed_notified;