CREATE OR REPLACE VIEW clevertap.task_board_viewed_events AS
...
SELECT
    ...
    -- task_board_viewed specific event properties
    element_at(eventProps, 'task_board_id').member5
        AS task_board_id,

    element_at(eventProps, 'task_board_title').member5
        AS task_board_title,

    element_at(eventProps, 'task_active_count').member5
        AS task_active_count,

    element_at(eventProps, 'task_completed_count').member5
        AS task_completed_count,

    element_at(eventProps, 'task_milestone_available_count').member5
        AS task_milestone_available_count,

    element_at(eventProps, 'task_current_points').member5
        AS task_current_points,
...
FROM clevertap.task_board_viewed;